...
(function(window) {
	...
	// begin change for hystrix-circuit-breaker plugin
	jQuery.get($('#hystrixCircuit').text(), function(data) {
		hystrixTemplateCircuit = data;
	});
	jQuery.get($('#hystrixCircuitContainer').text(), function(data) {
	...
	// end change for hystrix-circuit-breaker plugin
	...
	window.HystrixCommandMonitor = function(containerId, args) {
		...
		/* private */ function updateSparkline(variablePrefix, cssTarget, newDataPoint) {
				...
				if(data.length == 1 && data[0].v == 0) {
					//log("we have a single 0 so skipping");
					// don't show if we have a single 0
					return;
				}
				
				if(data.length > 1 && data[0].v == 0 && data[1].v != 0) {
					//log("we have a leading 0 so removing it");
